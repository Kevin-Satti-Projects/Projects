...
function removeProjectButtonClickEvents() {
  ...
  for (let i = 0; i < document.getElementById("tabs").childElementCount; i++) {
    buttons.push(
      document.getElementById("tabs").getElementsByTagName("button")[i]
    );
    document.getElementById("tabs").getElementsByTagName("button")[i].id =
      "Project" + String(Number(i + 1)) + "Button";
...
function clickedDeleteProjectButton() {
  if (!(document.querySelector(".active") == null)) {
    let check = confirm(
      "Delete Project: " + document.querySelector(".active").innerHTML + "?"
    );
    if (check == true) {
      document.querySelectorAll("li").forEach((li) => {
        li.parentNode.removeChild(li);
      ...
      let currentProjectNumber = Array.prototype.indexOf.call(
